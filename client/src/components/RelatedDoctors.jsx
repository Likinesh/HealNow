...
const RelatedDoctors = ({ docId, speciaility }) => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-white md:mx-10'>
      ...
      <div className='w-full grid grid-cols-3 { grid-template-columns: repeat(5, minmax(0, 1fr)); } gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {relDoc.slice(0, 5).map((item, index) => (
          <div key={index} onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='border border-blue-400 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
            ...
            <div className='p-4'>
              <div className={`flex items-center gap-2 text-sm text-center ${item.available ? 'text-green-500' : 'text-white'} `}>
                <p className={`w-2 h-2 ${item.available ? ' bg-green-500' : 'bg-white'} rounded-full`}></p><p>{item.available ? 'Available' : 'Not Available'}</p>
              </div>
      ...
      <button className='bg-blue-500 p-4 rounded-full' onClick={() => { navigate('/doctors'); scrollTo(0, 0) }}>To see all doctors</button>
    </div>
  )
}
